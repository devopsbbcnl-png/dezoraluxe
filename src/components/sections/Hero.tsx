...
const Hero = () => {
	const [heroProductImage, setHeroProductImage] = useState<string | null>(null);

	useEffect(() => {
		const loadHeroProduct = async () => {
			try {
				// Try to get the first featured product with an image
				...
				if (!error && data && data.images && data.images.length > 0) {
					setHeroProductImage(
						getOptimizedCloudinaryUrl(data.images[0], {
							width: 800,
							height: 800,
							crop: 'fill',
							quality: 'auto',
				...
				<div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
					...
					<div className="space-y-8 text-center lg:text-left">
						...
						<div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start opacity-0 animate-fade-up delay-300">
							...
							<Button variant="hero-outline" size="xl" asChild>
								<Link to="/categories">Explore Dezora Luxe</Link>
							</Button>
						</div>
					...
					<div className="relative flex items-center justify-center opacity-0 animate-scale-in delay-200">
						<div className="relative">
							...
							<img
								src={heroProductImage || heroProduct}
								alt="Premium Product"
